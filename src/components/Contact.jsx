...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-32 bg-base relative">
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <ScrollReveal3D direction="flip" delay={0} className="mb-16">
          <p className="text-accent text-[10px] tracking-[0.4em] uppercase mb-4">Let's talk</p>
          ...
          <ScrollReveal3D direction="left" delay={0.1}>
            <div className="flex flex-col gap-10">
              <p className="text-white/35 text-base leading-relaxed font-light">
                ...
                interesting conversations and collaborations. Drop me a message.
              </p>

              <div className="flex flex-col gap-5">
                {[
                  { label: 'Email', value: 'sheronovakbarali@gmail.com' },
                  { label: 'Location', value: 'Tashkent, Uzbekistan' },
                  { label: 'Status', value: 'Open to work' },
                ].map((item) => (
                  <div key={item.label} className="flex flex-col gap-1">
                    <p className="text-[10px] text-white/20 tracking-[0.35em] uppercase">{item.label}</p>
              ...
              <div>
                <p className="text-[10px] text-white/20 tracking-[0.35em] uppercase mb-4">Find me on</p>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] text-white/20 tracking-[0.35em] uppercase">Name</label>
                  <input type="text" name="name" value={form.name} onChange={handleChange}
                    placeholder="Akbarali Sheronov" required className={inputClass} />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] text-white/20 tracking-[0.35em] uppercase">Email</label>
                  <input type="email" name="email" value={form.email} onChange={handleChange}
                    placeholder="sheronovakbarali@gmail.com" required className={inputClass} />
                </div>
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-[10px] text-white/20 tracking-[0.35em] uppercase">Message</label>
              ...
              >
                ...
                ) : status === 'sent' ? '✓ Sent!' : status === 'error' ? '✗ Error, try again' : 'Send Message'}
              </motion.button>
            </form>
          </ScrollReveal3D>
