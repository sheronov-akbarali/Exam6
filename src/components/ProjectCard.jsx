...
export default function ProjectCard({ project }) {
  ...
  return (
    ...
    >
      <div className={`border transition-colors duration-300 bg-[#111] ${
        ...
        <div className="relative h-52 overflow-hidden bg-[#0e0e0e]">
          <div
            ...
            style={{ background: project.gradient, transform: hovered ? 'scale(1.04)' : 'scale(1)' }}
          />
          <div className="absolute inset-0 flex items-center justify-center">
            {project.logo
              ? <img src={project.logo} alt={project.title} className="w-32 h-32 object-contain opacity-90" />
              : <span className="text-3xl opacity-50">{project.icon}</span>
            }
          </div>
          {/* Bottom line reveal */}
          <div className={`absolute bottom-0 left-0 h-px bg-white/30 transition-all duration-400 ${hovered ? 'w-full' : 'w-0'}`} />
        </div>

        {/* Body */}
        <div className="p-6 flex flex-col gap-3">
          <div className="flex items-start justify-between gap-3">
            <h3 className="text-base font-semibold text-white/80 leading-snug">{project.title}</h3>
            <div className="flex gap-1 shrink-0 mt-0.5">
              ...
              {project.live && (
                <a href={project.live} target="_blank" rel="noopener noreferrer"
                  onClick={e => e.stopPropagation()} aria-label="Live"
                  className="p-1 text-white/20 hover:text-white/60 transition-colors">
          ...
          <p className="text-xs text-white/25 leading-relaxed font-light">{project.description}</p>

          <div className="flex flex-wrap gap-1.5 pt-0.5">
            {project.tags.map(tag => (
              <span key={tag} className="px-2 py-0.5 text-[11px] text-white/20 border border-white/[0.06] tracking-wide">
                {tag}
              </span>
            ))}
          </div>
